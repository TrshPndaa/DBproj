...
def update_course(course_id):
    ...
    try:
        data = request.json
        if not all(key in data for key in ['courseName', 'courseDescription', 'credits']):
        ...
        cursor.execute('''
            UPDATE course
            SET courseName = ?, courseDescription = ?, credits = ?
            WHERE id = ?
        ''', (data['courseName'], data['courseDescription'], data['credits'], course_id))
...
def validate_request(data):
    required = ['firstName', 'lastName', 'email', 'dateOfBirth', 'address', 'phoneNumber']
    if missing := [f for f in required if not data.get(f)]:
        return {"error": "Missing fields", "missing": missing}, False
    return {k: str(data[k]).strip() for k in required}, True
...
def create_student():
    try:
        ...
        data, valid = validate_request(request.get_json())
        if not valid:
            return jsonify(data), 400
            
        if empty := [k for k, v in data.items() if not v.strip()]:
            return jsonify({"error": "Empty fields", "fields": empty}), 400
